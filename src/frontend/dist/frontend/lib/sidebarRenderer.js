import { FUNCTION_LABELS, LEVEL_LABELS, SERIOUSNESS_LABELS, STUDENT_PRIORITY_LABELS, } from "../../shared/constants.js";
...
function renderCitations(_citations) {
    return "";
}
function getCourseMaterial(item) {
    const courseMaterial = item.citations.find((citation) => citation.type === "course_material");
...
function renderLearningModule(item) {
    const material = getCourseMaterial(item);
    const label = escapeHtml(material.label);
    const materialLink = material.url && material.url.length > 0
...
function seriousnessClass(seriousness) {
    switch (seriousness) {
        case "Extra high":
...
export function renderSidebarCards(annotations) {
    ...
    return annotations
        ...
        const priorityLabel = item.student_priority
            ...
            : null;
        const seriousnessMeta = item.seriousness && item.student_priority
...
export function renderSummary(overall) {
