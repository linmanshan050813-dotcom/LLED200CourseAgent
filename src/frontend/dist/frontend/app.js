...
import { activate, filterAnnotations, initialViewerState, setFunctionFilter, setLevelFilter, switchTab, } from "./lib/interactions.js";
...
function startFeedbackProgress() {
    let percent = 45;
...
function readFeedbackFromStorage() {
...
function applyActiveState(activeId) {
    document.querySelectorAll(".hl, .annotation-pin, .feedback-card").forEach((node) => {
        node.classList.remove("is-active");
    });
    if (activeId === null)
        return;
    document.querySelectorAll(`[data-id="${activeId}"]`).forEach((node) => {
...
function bindCrossHighlight(ctx) {
    document.querySelectorAll(".hl, .annotation-pin, .feedback-card").forEach((node) => {
        node.addEventListener("click", () => {
            ...
            const highlight = document.querySelector(`.hl[data-id="${id}"]`);
...
function renderFeedback(ctx) {
    ...
    tabsRoot?.querySelectorAll(".fb-tab").forEach((node) => {
        node.classList.toggle("is-active", node.dataset.tab === ctx.state.currentTab);
    });
    fnFiltersRoot?.querySelectorAll(".dim-pill").forEach((node) => {
        node.classList.toggle("is-active", node.dataset.filter === ctx.state.functionFilter);
    });
    levelFiltersRoot?.querySelectorAll(".dim-pill").forEach((node) => {
...
async function loadFileIntoContext(file, ctx, statusEl) {
...
function setupDropzone(ctx, statusEl, fileInput) {
    const dropZone = document.getElementById("dropZone");
    if (!dropZone)
        return;
...
function setupAppPage() {
    ...
    levelFiltersRoot?.querySelectorAll(".dim-pill").forEach((node) => {
        node.addEventListener("click", () => {
            const value = node.dataset.level;
            if (!value)
                return;
